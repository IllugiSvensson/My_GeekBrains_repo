...
FindPIDs=`pgrep -f "change_workstation_role.sh"`	#Ищем PID процесса(ов)
...
#for LIST in /system/etc/$CONFIG/*.xml
#do

#	List=`/system/bin/regtool.bin -n $LIST /Hosts`		#Читаем хост лист
#	HostList="$HostList $List"

#done
#CountOfHosts=`echo $HostList | grep -o " " | wc -l`
#ProgressBarIncrement="$((100 / $(($CountOfHosts + 1))))"; inc=0		#Инкремент хода выполнения проверки (для визуализации)
#touch /soft/scripts/CWR_online.txt	#Временный файл со списком онлайн хостов
#touch /soft/scripts/CWR_offline.txt	#Временный файл со списком оффлайн хостов

#Блок проверки наличия хостов в сети
#for HOST in $HostList		#Пингуем хосты из списка
#do

#	echo $inc
#	ping -c 1 -W 1 $HOST	#Добавляем в списки соответствующие хосты
#	if [ $? == 0 ]
#	then

#		HostListOnline="$HostListOnline $HOST"
#		echo $HostListOnline > /soft/scripts/CWR_online.txt

#	elif [ $? != 0 ]
#	then

#		HostListOffline="$HostListOffline $HOST"
#		echo $HostListOffline > /soft/scripts/CWR_offline.txt

#	fi
#	inc="$(($inc + $ProgressBarIncrement))"		#Дополняем прогрессбар на величину инкремента
